...
function CarouselItem({ article }) {
  return (
    // "a" used if to get to the source of the news
     <a href={article.url} className="embla__container h-full">
      <article className="bg-slate-100 dark:bg-slate-800 flex flex-col rounded-lg shadow-lg hover:scale-105 hover:shadow-xl hover:bg-slate-200 transition-all duration-200 ease-out embla__slide items-center justify-center">
        {article.image && (
          <img
            src={article.image}
            alt={article.title}
